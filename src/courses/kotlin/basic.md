...
在電腦的世界中，我們使用 0,1 來儲存資料，也就是二進位，像是 $10_{(10)}=1010_{(2)}$，
...
這在某些情況下可能會導致程式的執行效率降低，特別是在需要將大量的資料進行輸入輸出的情況下，

因為在競賽程式中，我們通常只會比較最後輸出的結果，
所以其實我們可以不用一邊輸入一邊輸出，而是等到所有輸入都完成後再一次性輸出結果就可以了。

為了達到這個目的，我們引入了 `BufferedReader` 和 `BufferedWriter` 這兩個工具。

`BufferedReader` 用來讀取輸入，而 `BufferedWriter` 用來輸出結果。
...
`br.readline()` 用來讀取一行輸入，`bw.write()` 用來寫入輸出，而 `bw.flush()` 則是將緩衝區的內容輸出到螢幕上，
對照原本內建的 `readLn()` 和 `print()`。
...
> **注意：**
> 在 Kotlin 中，整數除法會直接丟掉小數部分，等同於無條件捨去，舉例來說 5 / 3 = 1。如果需要得到浮點數結果，請使用 `Double` 型態，或將整數轉為浮點數後再計算。
