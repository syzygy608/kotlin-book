...
但是這樣的寫法跟 C++ 的輸出入緩衝有點類似，我們會總是等待使用者輸入完畢後才會繼續執行程式。這在某些情況下可能會導致程式的執行效率降低，特別是在需要頻繁輸入輸出的情況下，但是在競賽程式中，我們通常只會比較最後輸出的結果，所以其實我們可以不用一邊輸入一邊輸出，而是等到所有輸入都完成後再一次性輸出結果。

為了達到這個目的，我們引入了 `BufferedReader` 和 `PrintWriter` 這兩個類別。`BufferedReader` 用來讀取輸入，而 `PrintWriter` 用來輸出結果。這樣可以提高程式的執行效率，特別是在需要大量輸入輸出的情況下。

```kotlin
import java.io.*

// 使用 BufferedReader 和 PrintWriter 來提高輸入輸出效率
...
因為右移會直接把位元丟掉，所以如果是奇數的話，相當於除以 $2^k$ 之後會向下取整。

假設 `a = -12`，在二進位中，我們先將 $12$ 轉換為二進位 `0000 1100`，然後取反得到 `1111 0011`，再加一得到 `1111 0100`，這就是 -12 的二進位表示。
當我們使用 `shr` 進行右移時，符號位會被保留，這意味著如果我們將 `-12` 右移兩位，結果仍然是負數，並且符號位會被保留。
```kotlin
val a = -12 // 整數 -12
val result = a shr 2 // 右移兩位
writeln("右移兩位結果：${result.toString(2)}") // 輸出 -11 代表 -3
```

##### 取反運算（`inv`）
取反運算是將數字的每個位元進行取反操作，0 變成 1，1 變成 0。
```kotlin
val a = 0b1100 // 二進位表示的 12
val result = a.inv() // 位元取反運算
writeln("位元取反運算結果：${result.toString(2)}") // 輸出 0011
...
如果你有學過其他程式語言，可能會很好奇為什麼 Kotlin 沒有提供 `<<` 和 `>>` 這樣的位元移位運算符，這是因為 Kotlin 的設計者認為使用 `shl` 和 `shr` 更加清晰和易於理解，同時 `&`, `|` 等符號在 Kotlin 中已經被用來進行邏輯運算，所以使用 `and`, `or`, `xor` 等函數來進行位元運算可以避免混淆，
跟邏輯運算完全區分開來。
